...
void DialogGroup::ReadUILayout()
{
	...
	numeric = gconf_client_get_int(client,
				       GCONF_PATH "/group_historyinput_paned_divide",
				       NULL);
	...
	numeric = gconf_client_get_int(client,
				       GCONF_PATH "/group_memberenclosure_paned_divide",
				       NULL);
	numeric = numeric ? numeric : 320;
...
GtkTreeModel *DialogGroup::CreateMemberModel()
{
	...
	gtk_tree_sortable_set_default_sort_func(GTK_TREE_SORTABLE(model),
						...
						(MemberTreeCompareByNameFunc), NULL,
						NULL);
...
GtkWidget *DialogGroup::CreateToolMenu()
{
	...
	/* 清空历史缓冲区 */
	NO_OPERATION_C menuitem = gtk_menu_item_new_with_label(_("Clear Buffer"));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	g_signal_connect_swapped(menuitem, "activate",
				 G_CALLBACK(ClearHistoryBuffer), this);

	/* 群组成员排序 */
	NO_OPERATION_C menuitem = gtk_menu_item_new_with_label(_("Sort"));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	...
	do {
		...
		if (active) {
			if (FLAG_ISSET(pal->flags, 0)) {
				...
				cmd.SendUnitMsg(cthrd.UdpSockQuote(), pal, opttype, msg);
			} else
