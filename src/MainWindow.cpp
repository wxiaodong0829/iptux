...
void MainWindow::UpdateItemToTransTree(GData ** para)
{
	...
	if (para != data) {
		...
		gtk_list_store_set(GTK_LIST_STORE(model), &iter, TRANS_TREE_MAX, para,
				   -1);
	}
	...
	if (!g_datalist_get_data(para, "data"))
		gtk_list_store_set(GTK_LIST_STORE(model), &iter, TRANS_TREE_MAX, NULL,
				   -1);
...
GtkWidget *MainWindow::CreateToolMenu()
{
	...
	/* 群组成员排序 */
	NO_OPERATION_C menuitem = gtk_image_menu_item_new_with_mnemonic(_("_Sort"));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	...
	/*/* 按IP地址排序 */
	menuitem =
	    gtk_radio_menu_item_new_with_label_from_widget(GTK_RADIO_MENU_ITEM(menuitem),
							   _("By IP"));
	g_object_set_data(G_OBJECT(menuitem), "compare-func",
	...
	/* 更新成员 */
	NO_OPERATION_C menuitem = gtk_image_menu_item_new_with_mnemonic(_("_Update"));
...
{
	const char *faq = _("http://code.google.com/p/iptux/wiki/FAQ_EnglishVersion");
	GtkWidget *menushell;
	GtkWidget *menu, *menuitem;
...
GtkTreeModel *MainWindow::CreatePaltreeModel()
{
	...
	model = gtk_tree_store_new(7, GDK_TYPE_PIXBUF, GDK_TYPE_PIXBUF,
				   ...
				   PANGO_TYPE_ATTR_LIST, GDK_TYPE_COLOR, G_TYPE_POINTER);
	gtk_tree_sortable_set_default_sort_func(GTK_TREE_SORTABLE(model),
						GtkTreeIterCompareFunc
						(PaltreeCompareByNameFunc), NULL, NULL);
...
GtkWidget *MainWindow::CreatePaltreeTree(GtkTreeModel * model)
{
	...
	gtk_tree_view_column_pack_start(column, cell, FALSE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column), cell,
					    "text", 2, "attributes", 4, "foreground-gdk",
					    5, NULL);
	/* 扩展信息区域 */
	cell = gtk_cell_renderer_text_new();
	g_object_set(cell, "xalign", 0.0, "wrap-mode", PANGO_WRAP_WORD, NULL);
	gtk_tree_view_column_pack_start(column, cell, FALSE);
	gtk_tree_view_column_set_attributes(GTK_TREE_VIEW_COLUMN(column), cell,
					    "text", 3, "attributes", 4, "foreground-gdk",
					    5, NULL);
...
void MainWindow::BlinkGroupItemToPaltree(GtkTreeModel * model, GtkTreeIter * iter,
					 bool blinking)
{
	static GdkColor color1 = { 0xff, 0x5252, 0xb8b8, 0x3838 }, color2 = {
	0xff, 0x0000, 0x0000, 0xffff};
	GdkColor *color;
...
GtkWidget *MainWindow::CreateTransPopupMenu(GtkTreeModel * model)
{
	...
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	g_signal_connect_swapped(menuitem, "activate", G_CALLBACK(OpenThisFile), model);
	...
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	g_signal_connect_swapped(menuitem, "activate", G_CALLBACK(ClearTransTask), model);
...
GtkWidget *MainWindow::CreatePaltreePopupMenu(GroupInfo * grpinf)
{
	...
	/* 发送消息菜单 */
	NO_OPERATION_C menuitem = gtk_menu_item_new_with_label(_("Send Message"));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	switch (grpinf->type) {
	case GROUP_BELONG_TYPE_REGULAR:
		g_signal_connect_swapped(menuitem, "activate",
	...
	case GROUP_BELONG_TYPE_BROADCAST:
		g_signal_connect_swapped(menuitem, "activate",
					 G_CALLBACK(DialogGroup::GroupDialogEntry),
					 grpinf);
		break;
	default:
	...
	/* 改变好友信息数据菜单 */
	NO_OPERATION_C menuitem = gtk_menu_item_new_with_label(_("Change Info."));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	switch (grpinf->type) {
	case GROUP_BELONG_TYPE_REGULAR:
		g_signal_connect_swapped(menuitem, "activate",
	...
	/* 删除好友项菜单 */
	NO_OPERATION_C menuitem = gtk_menu_item_new_with_label(_("Delete Pal"));
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
	switch (grpinf->type) {
	case GROUP_BELONG_TYPE_REGULAR:
		g_signal_connect_swapped(menuitem, "activate",
	...
	if (filename) {
		if (!g_file_test(filename, G_FILE_TEST_EXISTS)) {
			GtkWidget *dialog = gtk_message_dialog_new(NULL,
								   ...
								   GTK_BUTTONS_OK,
								   _
								   ("The file you want to open not exist!"));
	...
	if (filename) {
		...
		if (!g_file_test(filepath, G_FILE_TEST_EXISTS)) {
			GtkWidget *dialog = gtk_message_dialog_new(NULL,
								   ...
								   GTK_BUTTONS_OK,
								   _
								   ("The path you want to open not exist!"));
			gtk_window_set_title(GTK_WINDOW(dialog), "Iptux Error");
			gtk_dialog_run(GTK_DIALOG(dialog));
			gtk_widget_destroy(dialog);
			return;
...
gboolean MainWindow::PaltreePopupMenu(GtkWidget * treeview, GdkEventButton * event)
{
	...
	if (event->button != 3
	    || !gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(treeview),
					      GINT(event->x), GINT(event->y), &path, NULL,
					      NULL, NULL))
		return FALSE;
 ...
 * @param dtset data set
 */
void MainWindow::PanedDivideChanged(GtkWidget * paned, GParamSpec * pspec, GData ** dtset)
{
