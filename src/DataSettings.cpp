...
GtkWidget *DataSettings::CreatePersonal()
{
	...
	/* 个人形象照片 */
	NO_OPERATION_C frame = gtk_frame_new(_("Photo"));
	...
	/* 个性签名 */
	NO_OPERATION_C frame = gtk_frame_new(_("Signature"));
	gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_ETCHED_IN);
...
GtkWidget *DataSettings::CreateSystem()
{
	...
	/* 有消息时直接弹出聊天窗口 */
	widget = gtk_check_button_new_with_label(_("Automatically open the chat dialog"));
	...
	/* 清空聊天历史记录 */
	widget =
	    gtk_check_button_new_with_label(_("Automatically clean up the chat history"));
	gtk_box_pack_start(GTK_BOX(box), widget, FALSE, FALSE, 0);
...
GtkWidget *DataSettings::CreateSound()
{
	...
	/* 声音事件 */
	NO_OPERATION_C frame = gtk_frame_new(_("Sound Event"));
	gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_ETCHED_IN);
...
GtkWidget *DataSettings::CreateNetwork()
{
	...
	/* 网段树&实用性按钮 */
	NO_OPERATION_C frame = gtk_frame_new(_("Added IP(v4) Section:"));
	gtk_frame_set_shadow_type(GTK_FRAME(frame), GTK_SHADOW_ETCHED_IN);
...
GtkTreeModel *DataSettings::CreateNetworkModel()
{
	...
	gtk_tree_sortable_set_default_sort_func(GTK_TREE_SORTABLE(model),
						...
						(NetworkTreeCompareFunc), NULL, NULL);
...
void DataSettings::FillSndModel(GtkTreeModel * model)
{
	...
	gtk_list_store_set(GTK_LIST_STORE(model), &iter,
			   ...
			   1, _("Transfer finished"), 2, progdt.transtip, -1);
	gtk_list_store_append(GTK_LIST_STORE(model), &iter);
	gtk_list_store_set(GTK_LIST_STORE(model), &iter,
			   0, FLAG_ISSET(progdt.sndfgs, 1),
			   1, _("Message received"), 2, progdt.msgtip, -1);
}
...
GtkWidget *DataSettings::CreateNetworkTree(GtkTreeModel * model)
{
	...
	cell = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes(_("To"), cell, "text", 1, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
...
GtkWidget *DataSettings::CreatePopupMenu(GtkTreeModel * model)
{
	...
	g_signal_connect_swapped(menuitem, "activate", G_CALLBACK(model_turn_all), model);
	gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
...
void DataSettings::AddNewIcon(GtkWidget * button, GData ** widset)
{
	...
	if (!
	    (filename =
	     choose_file_with_preview(_("Please select a face picture"), parent)))
		return;
...
void DataSettings::ChoosePhoto(GData ** widset)
{
	...
	if (!
	    (filename =
	     choose_file_with_preview(_("Please select a personal photo"), parent)))
		return;
