...
:  widset(NULL), mdlset(NULL), dtset(NULL), accel(NULL), grpinf(grp), totalsendsize(0)
{
	InitSublayerGeneral();
}
	...
	while (tlist) {
		...
		while (pallist) {
			...
			gtk_list_store_set(GTK_LIST_STORE(model), &iter, 0, pixbuf, 1,
					   filename, 2, numeric_to_size(filesize), 3,
					   tlist->data, 4, file, 5, file->fileown->name,
					   -1);
...
GtkWidget *DialogBase::CreateHistoryArea()
{
	...
	g_signal_connect(widget, "event-after", G_CALLBACK(textview_event_after), NULL);
 ...
 * @param dtset data set
 */
void DialogBase::PanedDivideChanged(GtkWidget * paned, GParamSpec * pspec, GData ** dtset)
{
...
GtkWidget *DialogBase::CreateFileSendTree(GtkTreeModel * model)
{
	...
	if (grpinf->type != GROUP_BELONG_TYPE_REGULAR) {
		...
		column =
		    gtk_tree_view_column_new_with_attributes(_("PeelName"), cell, "text",
							     5, NULL);
		gtk_tree_view_column_set_resizable(column, TRUE);
		gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
	}
	...
	cell = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Name"), cell, "text", 1, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);

	cell = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Size"), cell, "text", 2, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);

	cell = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Path"), cell, "text", 3, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
