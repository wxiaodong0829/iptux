 ...
 * @param grp 好友群组信息
 */
DialogPeer::DialogPeer(GroupInfo * grp):DialogBase(grp), torcvsize(0), rcvdsize(0)
{
	ReadUILayout();
}
...
void DialogPeer::ReadUILayout()
{
	...
	numeric = gconf_client_get_int(client,
				       GCONF_PATH "/peer_historyinput_paned_divide",
				       NULL);
	...
	numeric = gconf_client_get_int(client,
				       GCONF_PATH "/peer_infoenclosure_paned_divide",
				       NULL);
	numeric = numeric ? numeric : 255;
	...
	g_datalist_set_data(&dtset, "enclosure-paned-divide", GINT_TO_POINTER(numeric));

	numeric = gconf_client_get_int(client,
				       GCONF_PATH "/peer_file_recieve_paned_divide",
				       NULL);
...
void DialogPeer::InsertPicture(DialogPeer * dlgpr)
{
	...
	if (!
	    (filename =
	     choose_file_with_preview(_("Please select a picture to insert the buffer"),
				      window)))
		return;
...
GtkWidget *DialogPeer::CreateFileArea()
{
	...
	position = GPOINTER_TO_INT(g_datalist_get_data(&dtset, "enclosure-paned-divide"));
	gtk_paned_set_position(GTK_PANED(vpaned), position);
	g_signal_connect(vpaned, "notify::position",
			 G_CALLBACK(PanedDivideChanged), &dtset);
...
GtkWidget *DialogPeer::CreateFileToReceiveArea()
{
	...
	g_signal_connect_swapped(button, "clicked", G_CALLBACK(ReceiveFile), this);
	g_datalist_set_data(&widset, "file-receive-accept-button", button);
	gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, TRUE, 0);
	button = gtk_button_new_with_label(_("Refuse"));
	gtk_box_pack_start(GTK_BOX(hbox), button, FALSE, TRUE, 0);
	g_signal_connect_swapped(button, "clicked", G_CALLBACK(RemoveSelectedRcv), this);
...
GtkWidget *DialogPeer::CreateFileToReceiveTree(GtkTreeModel * model)
{
	...
	column =
	    gtk_tree_view_column_new_with_attributes(_("Source"), cell, "text", 1, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
	...
	column =
	    gtk_tree_view_column_new_with_attributes(_("SaveAs"), cell, "text", 2, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);

	cell = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Size"), cell, "text", 3, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
...
GtkWidget *DialogPeer::CreateFileReceivedTree(GtkTreeModel * model)
{
	...
	cell = gtk_cell_renderer_text_new();
	column =
	    gtk_tree_view_column_new_with_attributes(_("Name"), cell, "text", 2, NULL);
	gtk_tree_view_column_set_resizable(column, TRUE);
	gtk_tree_view_append_column(GTK_TREE_VIEW(view), column);
...
void DialogPeer::ShowInfoEnclosure(DialogPeer * dlgpr)
{
	...
	mdltorcv =
	    (GtkTreeModel *) g_datalist_get_data(&(dlgpr->mdlset),
						 "file-to-receive-model");
	gtk_list_store_clear(GTK_LIST_STORE(mdltorcv));
	mdlrcvd =
	    (GtkTreeModel *) g_datalist_get_data(&(dlgpr->mdlset), "file-received-model");
	...
	if (ecslist) {
		...
		widget =
		    GTK_WIDGET(g_datalist_get_data
			       (&(dlgpr->widset), "file-enclosure-frame-widget"));
		gtk_paned_pack2(GTK_PANED(hpaned), widget, FALSE, TRUE);
		widget =
		    GTK_WIDGET(g_datalist_get_data
			       (&(dlgpr->widset), "file-receive-paned-widget"));
		gtk_widget_show(widget);
		...
		while (ecslist) {
			...
			if (file->finishedsize < file->filesize) {
				file->filepath =
				    ipmsg_get_filename_me(file->filepath, &path);
		...
		if (dlgpr->torcvsize == 0) {
			...
			if (dlgpr->rcvdsize == 0)
				...
				progress =
				    percent(dlgpr->rcvdsize, dlgpr->torcvsize) / 100;
...
bool DialogPeer::UpdataEnclosureRcvUI(DialogPeer * dlgpr)
{
	...
	if ((progress == 1) || (progress == 0)) {
		...
		button =
		    GTK_WIDGET(g_datalist_get_data
			       (&(dlgpr->widset), "file-receive-accept-button"));
...
void DialogPeer::ReceiveFile(DialogPeer * dlgpr)
{
	...
	widget =
	    GTK_WIDGET(g_datalist_get_data
		       (&(dlgpr->widset), "file-to-receive-treeview-widget"));
